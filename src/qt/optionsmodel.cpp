...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::IOI);
    ...
    if (!settings.contains("nAnonymize101coinAmount"))
        settings.setValue("nAnonymize101coinAmount", 1000);

    nAnonymize101coinAmount = settings.value("nAnonymize101coinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymize101coinAmount"))
        SoftSetArg("-anonymize101coinamount", settings.value("nAnonymize101coinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case Anonymize101coinAmount:
            return QVariant(nAnonymize101coinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case Anonymize101coinAmount:
            nAnonymize101coinAmount = value.toInt();
            settings.setValue("nAnonymize101coinAmount", nAnonymize101coinAmount);
            emit anonymize101coinAmountChanged(nAnonymize101coinAmount);
            break;
        case CoinControlFeatures:
