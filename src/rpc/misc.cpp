...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total 101 Coin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in 101x/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in 101x/kb\n"
    ...
    UniValue z101Obj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        z101Obj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    z101Obj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("z101supply", z101Obj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given 101 Coin address.\n"

            "\nArguments:\n"
            "1. \"101coinaddress\"     (string, required) The 101 Coin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"101coinaddress\", (string) The  101 Coin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1:  101 Coin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are  101 Coin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string)  101 Coin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
